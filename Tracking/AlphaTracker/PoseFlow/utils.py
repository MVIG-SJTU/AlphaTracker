...
def best_matching_hungarian_newselect(
    all_cors,
    all_pids_info,
    all_pids_fff,
    track_vid_next_fid,
    ...
    for pid1 in range(box1_num):
        ...
        for pid2 in range(1, track_vid_next_fid["num_boxes"] + 1):
            ...
            if box1_fff:
                grade = cal_grade(
                    [dm_iou, box_iou, pose_iou_dm, pose_iou, box1_score, box2_score],
                    weights,
                )
            else:
                grade = cal_grade(
...
def best_matching_hungarian_noORB(
    all_cors,
    all_pids_info,
    all_pids_fff,
    track_vid_next_fid,
    ...
    for i in range(len(tracker.tracks)):
        if len(tracker.tracks[i].trace) > 1:
            for j in range(len(tracker.tracks[i].trace) - 1):
                ...
                cv2.line(
                    frame, (int(x1), int(y1)), (int(x2), int(y2)), track_colors[clr], 2
                )
    return frame
