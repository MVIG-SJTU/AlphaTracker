    ...
    for idx, frame_name in enumerate(tqdm(frame_list)):
        ...
        for pid in range(max_pid_id_setting, len(track_in[frame_name])):
            ...
            if track_in[frame_name][pid]["scores"] > min(current_select_score):
                ...
                current_select_score_pid_dict[
                    track_in[frame_name][pid]["scores"]
                ] = min_score_pid
...
def init_kfs(track_forJson, max_pid_id_setting, num_pose, init_id):
    ...
    for mouse_i in range(max_pid_id_setting):
        for pose_j in range(num_pose):
            for xOrY in range(2):
                pos_1 = track_forJson[fame_1_key][mouse_i]["keypoints"][
                    3 * pose_j + xOrY
                ]
                pos_0 = track_forJson[fame_0_key][mouse_i]["keypoints"][
                    3 * pose_j + xOrY
                ]
                velocity = pos_1 - pos_0
                confidence = 0.5 * (
                    track_forJson[fame_1_key][mouse_i]["keypoints"][3 * pose_j + 2]
                    + track_forJson[fame_1_key][mouse_i]["keypoints"][3 * pose_j + 2]
                )
                kfs[mouse_i].append(
                    pos_vel_filter(
                        ...
                        * (
                        ...
                        Q=0.1,  ## Q is the process covariance. white nose would be fine
                        dt=1,  ## time interval of two kalman update
                    )
                )
    return kfs
        ...
        if mouse_info_dict:
            for pose_j in range(int(len(mouse_info_dict["keypoints"]) / 3)):
                for xOrY in range(2):
                    ...
                    frame_info_list_pred[mouse_i]["keypoints"][
                        pose_j * 3 + xOrY
                    ] = kf.x[0]
...
def fill_blank_with_predict(track_forJson, frame_id, sorted_keys):
    ...
    for mouse_i in range(len(frame_info_list)):
        ...
        if not mouse_info_dict and frame_id > 3:
            ...
            mouse_info_dict["scores"] = (
                mouse0_info_dict["scores"] + mouse1_info_dict["scores"]
            ) / 2
            mouse_info_dict["keypoints"] = []
            mouse_info_dict["box"] = [999999, 99999, -1, -1]
            for pose_j in range(int(len(mouse1_info_dict["keypoints"]) / 3)):
                pred_x = (
                    2 * mouse1_info_dict["keypoints"][3 * pose_j + 0]
                    - mouse0_info_dict["keypoints"][3 * pose_j + 0]
                )
                pred_y = (
                    2 * mouse1_info_dict["keypoints"][3 * pose_j + 1]
                    - mouse0_info_dict["keypoints"][3 * pose_j + 1]
                )
                pred_s = (
                    mouse1_info_dict["keypoints"][3 * pose_j + 2]
                    + mouse0_info_dict["keypoints"][3 * pose_j + 2]
                ) / 2
...
def select_slowPose(frame_info_list_pred, track_forJson, frame_id, sorted_keys):
    ...
    for mouse_i in range(len(frame_info_list)):
        ...
        for pose_j in range(int(len(mouse0_info_dict["keypoints"]) / 3)):
            for xOrY in range(2):
                v_pred = abs(
                    mouse_info_dict_pred["keypoints"][3 * pose_j + xOrY]
                    - mouse0_info_dict["keypoints"][3 * pose_j + xOrY]
                )
                v = abs(
                    mouse_info_dict["keypoints"][3 * pose_j + xOrY]
                    - mouse0_info_dict["keypoints"][3 * pose_j + xOrY]
                )
                if v_pred > 2 * v:
                    frame_info_list_pred[mouse_i]["keypoints"][
...
def post_process_tracking(track_forJson, args):
    ...
    if args.kalman:
        kfs = init_kfs(
            track_forJson,
            args.max_pid_id_setting,
            args.num_pose,
            init_id=frame_start + 1,
        )
    if args.fill_blank_with_predict and args.max_pid_id_setting == -1:
        print(
            "Warning! If fill_blank_with_predict==True, then max_pid_id_setting would better not be -1."
        )
    ...
    for frame_id in tqdm(range(frame_start, frame_end)):
        ...
        if args.smooth_pose:
            frame_info_list_pred = select_slowPose(
                frame_info_list_pred, track_forJson, frame_id, sorted_keys
            )
        ...
        if args.fill_blank_with_predict:
            track_forJson = fill_blank_with_predict(
                track_forJson, frame_id, sorted_keys
            )
...
def display_track_forJson(track_forJson, args):
    img_tmp = cv2.imread(
        os.path.join(args.imgdir, args.image_format % (list(track_forJson.keys())[0]))
    )
    ...
    for frame_id in tqdm(range(len(sorted_keys))):
        ...
        for pid in range(len(track_forJson[imgname])):
            if not track_forJson[imgname][pid]:
                continue
            pose = np.array(track_forJson[imgname][pid]["keypoints"]).reshape(-1, 3)[
                :, :3
            ]
            for idx_c in range(pose.shape[0]):
                if pose[idx_c, 2] < joint_thred:
            ...
            for idx in range(len(pairs)):
                if (
                    pose[pairs[idx][0], 2] < joint_thred
                    or pose[pairs[idx][1], 2] < joint_thred
                ):
                    continue
                cv2.line(
                    img,
                    pt1=(
                        int(np.clip(pose[pairs[idx][0], 0], 0, width)),
                        int(np.clip(pose[pairs[idx][0], 1], 0, height)),
                    ),
                    pt2=(
                        int(np.clip(pose[pairs[idx][1], 0], 0, width)),
                        int(np.clip(pose[pairs[idx][1], 1], 0, height)),
                    ),
                    color=((160 * pid) % 255, (80 * pid) % 255, (30 * pid) % 255),
                    thickness=3,
                )
                # color=(255,0,255), thickness=3)
...
def display_pose_cv2(imgdir, visdir, track_forJson, kfs, cmap, args):
    ...
    img_tmp = cv2.imread(
        os.path.join(imgdir, args.image_format % (list(track_forJson.keys())[0]))
    )
    height, width, channels = img_tmp.shape
    fourcc = cv2.VideoWriter_fourcc(*"mp4v")  # Be sure to use lower case
    out = cv2.VideoWriter(args.out_video_path, fourcc, 20.0, (width, height))
    ...
    for imgname in tqdm(aa):
        ...
        for pid in range(len(track_forJson[imgname])):
            ...
            pose = np.array(frame_info_list_pred[pid]["keypoints"]).reshape(-1, 3)[
                :, :3
            ]
            for idx_c in range(pose.shape[0]):
                if pose[idx_c, 2] < joint_thred:
                ...
                cv2.circle(
                    ...
                    radius=6,
                    color=(0, (80 * idx_c) % 255, (120 * idx_c) % 255),
                    thickness=-1,
                )
            for idx in range(len(pairs)):
                if (
