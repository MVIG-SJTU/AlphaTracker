...
def display_pose_cv2(imgdir, visdir, tracked, cmap, args):
    ...
    img_tmp = cv2.imread(
        os.path.join(imgdir, args.image_format % (list(tracked.keys())[0]))
    )
    ...
    for idx, frame_name in enumerate(tqdm(frame_list)):
        ...
        for pid in range(max_pid_id_setting + 1, track_in[frame_name]["num_boxes"] + 1):
            ...
            if track_in[frame_name][pid]["box_score"] > min(current_select_score):
                ...
                current_select_score_pid_dict[
                    track_in[frame_name][pid]["box_score"]
                ] = min_score_pid
...
parser.add_argument(
    "--imgdir", type=str, required=True, help="Must input the images dir"
)
parser.add_argument(
    "--in_json", type=str, required=True, help="result json predicted by AlphaPose"
)
parser.add_argument(
    "--out_json", type=str, required=True, help="output path of tracked json"
)
parser.add_argument(
    "--visdir",
    type=str,
...
parser.add_argument(
    "--out_video_path", type=str, default="output.mp4", help="image foramt"
)
...
parser.add_argument(
    ...
    type=int,
    default=[1, 2, 1, 2, 0, 0],
    help="dm_iou, box_iou, pose_iou_dm, pose_iou, box1_score, box2_score",
)
...
if "forvis" not in notrack_json:
    ...
    notrack_json = os.path.join(
        os.path.dirname(notrack_json), "alphapose-results-forvis.json"
    )
...
with open(notrack_json, "r") as f:
    ...
    for img_name in tqdm(sorted(notrack.keys())):
        ...
        for bid in range(len(notrack[img_name])):
            ...
            track[img_name][bid + 1]["box_pos"] = [
                int(notrack[img_name][bid]["box"][0]),
    ...
    for pid1, pid2 in match_indexes:
        if match_scores[pid1][pid2] > match_thres:
            ...
            if track[next_frame_name][pid2 + 1]["new_pid"] > max_pid_id:
                print(
                    "tracking warning!!\n track[next_frame_name][pid2+1]['new_pid']>max_pid_id:",
                    next_frame_name,
