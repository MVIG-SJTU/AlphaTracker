...
def merge_clean_ln_split_Data(
    ...
    for ii in range(len(json_file_list)):
        ...
        for single_img_data in tqdm(data):
            try:
                ...
                for idx in range(len(annot)):
                    if (
                        annot[idx]["class"] == "Face"
                        or annot[idx]["class"] == "boundingBox"
                        or annot[idx]["class"] == "point"
                    ):
                ...
                if len(new_annot) != num_mouse[ii] * (num_pose + 1):
                    print(
                        "Bad annotation: there are %s animals and %s keypoints for each animal, but only %s annotation"
                ...
                if not has_box:
                    print(
                        "Bad annotation: the annotation does not contains bounding box"
                    )
                    num_badAnnot += 1
                    continue
                for mice_id in range(num_mouse[ii]):
                    ...
                    pt = [
                        [
                            annot[mice_id * (num_pose + 1) + k + 1]["x"],
                            annot[mice_id * (num_pose + 1) + k + 1]["y"],
                        ]
                        for k in range(num_pose)
                    ][:]
                ...
                os.system(
                    "ln -s {}/{} {}/{}".format(
                        image_root_list[ii],
                        single_img_data["filename"],
                        ln_image_dir,
                        "%04d" % (ii)
                        + "_"
                        + "%06d" % (single_img_data_count)
                        + single_img_data["filename"][-4:],
                    )
                )
                single_img_data["filename"] = (
                    "%04d" % (ii)
        ...
        num_allAnnot_train += (
            len(new_data[: int(len(data) * train_val_split)]) * num_mouse[ii]
        )
        num_allAnnot_valid += (
            len(new_data[int(len(data) * train_val_split) :]) * num_mouse[ii]
        )
        ...
        for vd in new_data[int(len(data) * train_val_split) :]:
            os.system(
                "ln -s {}/{} {}/{}".format(
                    ln_image_dir, vd["filename"], valid_image_root, vd["filename"]
                )
            )
...
def generate_yolo_data(
    list_file, data_in, image_root_in, yolo_annot_root, image_suffix, color_img_prefix
):
    with open(list_file, "w") as traintxt:
        for item in tqdm(data_in):
            ...
            with open(
                (
                    yolo_annot_root
                    + item["filename"].strip(item["filename"].split(".")[-1])
                    + "txt"
                ),
                "w",
            ) as fileout:
