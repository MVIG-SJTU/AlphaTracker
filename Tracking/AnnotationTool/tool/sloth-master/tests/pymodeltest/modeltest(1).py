...
class ModelTest(QtCore.QObject):
    def __init__(self, _model, parent):
        ...
        self.connect(
            self.model,
            QtCore.SIGNAL("columnsAboutToBeInserted(const QModelIndex&, int, int)"),
            self.runAllTests,
        )
        self.connect(
            self.model,
            QtCore.SIGNAL("columnsAboutToBeRemoved(const QModelIndex&, int, int)"),
            self.runAllTests,
        )
        self.connect(
            self.model,
            QtCore.SIGNAL("columnsInserted(const QModelIndex&, int, int)"),
            self.runAllTests,
        )
        self.connect(
            self.model,
            QtCore.SIGNAL("columnsRemoved(const QModelIndex&, int, int)"),
            self.runAllTests,
        )
        self.connect(
            self.model,
        ...
        self.connect(
            self.model,
            QtCore.SIGNAL("headerDataChanged(Qt::Orientation, int, int)"),
            self.runAllTests,
        )
        self.connect(
            self.model, QtCore.SIGNAL("layoutAboutToBeChanged()"), self.runAllTests
        )
        ...
        self.connect(
            self.model,
            QtCore.SIGNAL("rowsAboutToBeInserted(const QModelIndex&, int, int)"),
            self.runAllTests,
        )
        self.connect(
            self.model,
            QtCore.SIGNAL("rowsAboutToBeRemoved(const QModelIndex&, int, int)"),
            self.runAllTests,
        )
        self.connect(
            self.model,
    ...
    def data(self):
        ...
        if variant.isValid():
            ...
            assert (
                ...
                or state == QtCore.Qt.PartiallyChecked
                or state == QtCore.Qt.Checked
            )
