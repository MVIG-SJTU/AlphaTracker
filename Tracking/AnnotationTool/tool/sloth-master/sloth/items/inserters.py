...
class ItemInserter(QObject):
    ...
    def __init__(
        self, labeltool, scene, default_properties=None, prefix="", commit=True
    ):
...
class PointItemInserter(ItemInserter):
    def mousePressEvent(self, event, image_item):
        pos = event.scenePos()
        self._ann.update({self._prefix + "x": pos.x(), self._prefix + "y": pos.y()})
...
class RectItemInserter(ItemInserter):
    def __init__(
        ...
        ItemInserter.__init__(
            self, labeltool, scene, default_properties, prefix, commit
        )
        self._init_pos = None
    ...
    def mouseReleaseEvent(self, event, image_item):
        if self._item is not None:
            if self._item.rect().width() > 1 and self._item.rect().height() > 1:
...
class SequenceItemInserter(ItemInserter):
    ...
    def nextState(self, next_state=None):
        ...
        self._current_inserter = callable_(
            self._labeltool, self._scene, prefix=prefix, commit=False
        )
...
class BBoxFaceInserter(SequenceItemInserter):
    ...
    def imageChange(self):
        if self._state > 0:
            ...
            self._cleanup()
            self.nextState(0)
            self._scene.setMessage(
                "<b>Warning</b>: Image changed during insert operation.\n"
                + "Resetting the inserter state.\n"
                + "Now at: "
                + self.inserters[self._state][2]
            )
...
class PolygonItemInserter(ItemInserter):
    ...
    def _updateAnnotation(self):
        ...
        self._ann.update(
            {
                self._prefix + "xn": ";".join([str(p.x()) for p in polygon]),
                self._prefix + "yn": ";".join([str(p.y()) for p in polygon]),
            }
        )
        self._ann.update(self._default_properties)
