...
def get_samples(video_path, json_path, contour_path, arg):
    ...
    while read_flag:
        ...
        mouseIdUsed = []
        mouseMatchedMask = [
            {"mask": None, "maxOverlabJoinNum": 0, "contour": None}
            for iii in range(arg.mice_num)
        ]
        for contour_id, contour in enumerate(contours):
            if contour.size > 150:
                ...
                for mouseId in range(arg.mice_num):
                    overlabJoinNum = 0
                    pose = np.asarray(mouses[mouseId]["keypoints"]).reshape(
                        (arg.joint_num, 3)
                    )
        ...
        for targetMouseID in range(len(mouseMatchedMask)):
            if mouseMatchedMask[targetMouseID]["maxOverlabJoinNum"] > 0:
                pose1 = np.asarray(mouses[targetMouseID]["keypoints"]).reshape(
                    (arg.joint_num, 3)
                )
                ...
                final_crop_size = 200
                crop = rot[
                    int(width / 2 - ori_crop_halfSize) : int(
                        width / 2 + ori_crop_halfSize
                    ),
                    int(height / 2 - ori_crop_halfSize) : int(
                        height / 2 + ori_crop_halfSize
                    ),
                ].copy()
                crop = cv2.resize(
                    crop,
                ...
                cv2.imwrite(
                    contour_path + "/mask_mouse{}_{}.png".format(targetMouseID, i), crop
                )
            ...
            print(
                "\r counters images generated in:"
                + contour_path
                + ": {}".format(
                    [
                        "mask_mouse{}_{}.png".format(mid, i)
                        for mid in targetMouseIDGenerate
                    ]
                ),
                end="",
                flush=True,
    ...
    cap.release()
    print(
        "%d frames dont have enough mice, %d frames have points outside pic. %d counter is generated, %d frame in total"
        % (count_lessMouse, count_out, count_used, count_frame)
    )
