    ...
    while read_flag:
        ...
        for contour_id, contour in enumerate(contours):
            ...
            if (contour.size > 150) and (contour.size < 600):
                ...
                if (
                    (mask[int(pose1[0, 1]), int(pose1[0, 0]), 0] != 0)
                    and (mask[int(pose1[3, 1]), int(pose1[3, 0]), 0] != 0)
                    and (0 not in mouse_processed)
                ):
                    mouse_id = 0
                    mouse_processed.append(0)
                elif (
                    (mask[int(pose2[0, 1]), int(pose2[0, 0]), 0] != 0)
                    and (mask[int(pose2[3, 1]), int(pose2[3, 0]), 0] != 0)
                    and (1 not in mouse_processed)
                ):
                    mouse_id = 1
                    mouse_processed.append(1)
                elif (
                    (mask[int(pose1[0, 1]), int(pose1[0, 0]), 0] != 0)
                    or (mask[int(pose1[3, 1]), int(pose1[3, 0]), 0] != 0)
                    and (0 not in mouse_processed)
                ):
                    mouse_id = 0
                    mouse_processed.append(0)
                    ...
                    (mask[int(pose2[0, 1]), int(pose2[0, 0]), 0] != 0)
                    or (mask[int(pose2[3, 1]), int(pose2[3, 0]), 0] != 0)
                    and (1 not in mouse_processed)
                ):
                    mouse_id = 1
                    mouse_processed.append(1)
                ...
                cv2.imwrite(
                    contour_path + "/mask_{}_mouse_{}.png".format(i, mouse_id), crop
                )
                print(
                    "\r" + contour_path + "/mask_{}_mouse_{}.png".format(i, mouse_id),
                    end="",
                    flush=True,
                )
