...
for i in range(len(arg.videodir)):
    ...
    # ------------------- video for debug -----------------------
    out_video_path = "{}/contour_{}_debug.mp4".format(
        "./tmp", video_path.split("/")[-1].split(".")[0]
    )
    print("\n" + out_video_path)
            ...
            if (contour.size > 150) and (contour.size < 600):
                ...
                if flag:
                    ...
                    if flag2:
                        ...
                        # # plt.close()
                        crop = cv2.resize(
                            crop, (300, 300), interpolation=cv2.INTER_AREA
                        )
                        crop_f = cv2.resize(
                            crop_f, (300, 300), interpolation=cv2.INTER_AREA
                        )
                        ...
                        for pose in [pose1, pose2]:
                            ...
                            for idx in range(len(pairs)):
                                cv2.line(
                                    frame_ori,
                                    pt1=(
                                        int(np.clip(pose[pairs[idx][0], 0], 0, height)),
                                        int(np.clip(pose[pairs[idx][0], 1], 0, width)),
                                    ),
                                    pt2=(
                                        int(np.clip(pose[pairs[idx][1], 0], 0, height)),
                                        int(np.clip(pose[pairs[idx][1], 1], 0, width)),
                                ...
                                cv2.putText(
                                    ...
                                    text=str(tracked_id),
                                    org=(
                                        int(np.clip(pose[0, 0], 0, height)),
                                        int(np.clip(pose[0, 1], 0, width)),
                                    ),
                                    fontFace=cv2.FONT_HERSHEY_SIMPLEX,
                        ...
                        frame_ori = cv2.resize(
                            frame_ori, (800, 600), interpolation=cv2.INTER_AREA
                        )
                        ...
                        print(
                            "\r {} {}/mask_{}.png".format(show_count, contour_path, i),
                            end="",
                            flush=True,
                        )
