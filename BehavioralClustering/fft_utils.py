...
def select_clips(folder_dict, intervals, arg):
    ...
    while len(frames) >= arg.clip_length:
        flag = frames[0]
        if frames[arg.clip_length - 1] == flag + arg.clip_length - 1 and interval_in(
            start=frames[0], end=frames[1], intervals=intervals
        ):
...
def retrieve_poses(arg):
    ...
    for i in [1]:
        ...
        data_keys.sort(
            key=cmp_to_key(lambda a, b: int(a.split("_")[-1]) - int(b.split("_")[-1]))
        )
...
def retrieve_poses_Mice(arg):
    ...
    for i in range(min(arg.max_clips_num * 3, len(json_names))):
        ...
        if len(pose_clips) > arg.max_clips_num * 6:
            print(
                "max_clips_num is set to be {}, {} clips are retrieved. There should be enough clip left after cleaning data process. if not, please increase max_clips_num".format(
                    arg.max_clips_num, arg.max_clips_num * 6
                )
            )
            break
        ...
        info_dict = {
            "track_infos": [],
            "frame_keys": [],
            "video_path": arg.imgdir[i],
            "clip_id": "%d_%d" % (i, clip_id),
        }
        frame_ind_inClip = 0
        while frame_id < len(data_keys):
            print(
                "\rprocessing frame No.{} into clip No.{} as the {} frame".format(
                    frame_id, clip_id, frame_ind_inClip
                ),
                end="",
            )
            ...
            if data_key not in data_keys:
                print(
                    "can not find key {} in json. (If all the frame have this error. Please check json_keyFormat in setting.py)".format(
                        data_key
                    )
                )
                frame_id += 1
            ...
            try:
                mice = data[data_key]
                miceCenters = [
                    np.asarray(mice[m_id]["keypoints"])
                    .reshape((arg.joint_num, 3))
                    .mean(axis=0)
                    for m_id in range(len(mice))
                ]
                dist = [
                    distBetweenTwoMice(miceCenters[arg.targetMouseID], miceCenters[i])
                    for i in range(len(miceCenters))
                ]
                ...
                frame = plt.imread(
                    arg.contdir[i]
                    + "/"
                    + "mask_mouse{}_{}.png".format(arg.targetMouseID, frame_id)
                )
                frame = np.asarray(frame[:, :, 0], dtype="uint8")
                ...
                frame = plt.imread(
                    arg.contdir[i]
                    + "/"
                    + "mask_mouse{}_{}.png".format(referMouseID, frame_id)
                )
                frame = np.asarray(frame[:, :, 0], dtype="uint8")
                face = pd.Series(frame.flatten(), name=frame_id)
                faces = faces.append(face)
        ...
        for clip_ind in tqdm(range(clip_num)):
            ...
            for frame_ind in frame_inds[clip_ind, :]:
                ...
                try:
                    ...
                    if (
                        len(mice) <= 1
                        or "keypoints" not in mice[0]
                        or "keypoints" not in mice[1]
                    ):
                        # if(len(mice)<=1):
                        j = j + 1
                        continue
                ...
                frame = plt.imread(
                    arg.contdir[i]
                    + "mask_mouse{}_{}.png".format(arg.targetMouseID, frame_ind)
                )
                frame = np.asarray(frame[:, :, 0], dtype="uint8")
                face = pd.Series(frame.flatten(), name=frame_ind)
                faces = faces.append(face)
                ...
                frame = plt.imread(
                    arg.contdir[i]
                    + "mask_mouse{}_{}.png".format(referMouseID, frame_ind)
                )
                frame = np.asarray(frame[:, :, 0], dtype="uint8")
                face = pd.Series(frame.flatten(), name=frame_ind)
                faces = faces.append(face)
...
def clean_differentLength_clips(clips_dict):
    (
        pose_clips_filtered,
        poseTheOther_clips_filtered,
        info_clips_filtered,
        cont_clips_filtered,
        contTheOther_clips_filtered,
        frames_path_clips_filtered,
    ) = ([], [], [], [], [], [])
    pose_clips = clips_dict["pose_clips"]
    if len(pose_clips) == 0:
        print(
            "there are no clip to process. Please check if the script in ./utils_file run currectly and the json is correct"
        )
        raise
    ...
    return {
        "pose_clips": pose_clips_filtered,
...
def remove_longMiceDist_clips(cfg, clips_dict):
    ...
    for clip_ind in range(len(pose_clips)):
        ...
        bodyVector_xy_Nx2 = (
            pose_clips[clip_ind, :, 0, :] - pose_clips[clip_ind, :, 3, :]
        )
        if (
            distance
            < np.max(np.linalg.norm(bodyVector_xy_Nx2)) * cfg.distance_threshold
        ):
            pose_clips_filtered.append(clips_dict["pose_clips"][clip_ind])
            poseTheOther_clips_filtered.append(
                clips_dict["poseTheOther_clips"][clip_ind]
            )
            cont_clips_filtered.append(clips_dict["cont_clips"][clip_ind])
            contTheOther_clips_filtered.append(
                clips_dict["contTheOther_clips"][clip_ind]
            )
            ...
            numOfClipTooFar += 1
    print(
        "There are {} clips where mice are too far from each other. Removed".format(
            numOfClipTooFar
        )
    )
...
def compute_features_sep(arg, norm_pose_clips, info_clips, cont_clips):
    ...
    for clip_ind in range(clip_num):
        ...
        mid_nose, mid_ear1, mid_ear2, mid_tail = (
            mid_pose[0, 0:2],
            mid_pose[1, 0:2],
            mid_pose[2, 0:2],
            mid_pose[3, 0:2],
        ...
        head = (
            norm_pose_clips[clip_ind, :, 0, :]
            - (norm_pose_clips[clip_ind, :, 1, :] + norm_pose_clips[clip_ind, :, 2, :])
            / 2
        )
        ...
        point = 0
        vec = (
            norm_pose_clips[clip_ind, :, point, 0]
            + 1j * norm_pose_clips[clip_ind, :, point, 1]
        )
        np_fft = np.fft.fft(vec)
        amplitudes = 2 / vec.size * np.abs(np_fft)
        angles = np.angle(np_fft)
        ...
        head_body_ang_clips.append(
            np.concatenate((np.asarray(sin_angles), np.asarray(cos_angles)))
        )
        ...
        nose_fft_ang_clips.append(
            np.concatenate((np.sin(angles)[0:8], np.cos(angles)[0:8]))
        )
        contourPCA_fft_amp_clips.append(np.asarray(amplitudes_contour[0:8]))
        contourPCA_fft_ang_clips.append(
            np.concatenate((np.sin(angles_contour)[0:8], np.cos(angles_contour)[0:8]))
        )
...
def compute_features_sep_twoMice(
    arg,
    norm_pose_clips,
    norm_poseTheOther_clips,
    info_clips,
    cont_clips,
    pose_clips,
    poseTheOther_clips,
    ...
    for clip_ind in range(clip_num):
        ...
        body = norm_pose_clips[clip_ind, :, 0, :] - norm_pose_clips[clip_ind, :, 3, :]
        body_TO = (
            norm_poseTheOther_clips[clip_ind, :, 0, :]
            - norm_poseTheOther_clips[clip_ind, :, 3, :]
        )

        if (
            np.linalg.norm(mid_body) == 0
            or np.linalg.norm(mid_head) == 0
            or np.any(np.linalg.norm(body) == 0)
            or np.any(np.linalg.norm(body_TO) == 0)
        ):
        ...
        head_TO = (
            norm_poseTheOther_clips[clip_ind, :, 0, :]
            - (
                norm_poseTheOther_clips[clip_ind, :, 1, :]
                + norm_poseTheOther_clips[clip_ind, :, 2, :]
            )
            / 2
        )
        ...
        for i in range(arg.clip_length):
            ...
            if cos_angle == np.nan or sin_angle == np.nan:
                print(
                    "cos_angle==np.nan or sin_angle==np.nan", (body[i, :], head[i, :])
                )
        ...
        for i in range(arg.clip_length):
            ...
            if cos_angle == np.nan or sin_angle == np.nan:
                print(
                    "cos_angle==np.nan or sin_angle==np.nan",
                    (body_TO[i, :], head_TO[i, :]),
                )
        ...
        point = 0
        vec_TO = (
            norm_poseTheOther_clips[clip_ind, :, point, 0]
            + 1j * norm_poseTheOther_clips[clip_ind, :, point, 1]
        )
        ...
        distance_Tmouse_nose_to_Rmouse_tail_x = norm_poseTheOther_clips[
            clip_ind, :, 3, 0
        ]
        distance_Tmouse_nose_to_Rmouse_tail_y = norm_poseTheOther_clips[
            clip_ind, :, 3, 1
        ]
        ...
        head_earMid_length_clips.append(np.linalg.norm(head, axis=1))
        head_body_ang_clips.append(
            np.concatenate((np.asarray(sin_angles_hb), np.asarray(cos_angles_hb)))
        )
        displacementx_clips.append(np.asarray(displace_x))
        displacementy_clips.append(np.asarray(displace_y))
        displace_rho_clips.append(np.asarray(rho_vector))
        ...
        head_body_ang_clips_TO.append(
            np.concatenate((np.asarray(sin_angles_hb_TO), np.asarray(cos_angles_hb_TO)))
        )
        ...
        nose_fft_ang_clips_TO.append(
            np.concatenate((np.sin(angles_TO)[0:8], np.cos(angles_TO)[0:8]))
        )

        two_body_ang_clips.append(
            np.concatenate(
                (np.asarray(sin_angles_twoBody), np.asarray(cos_angles_twoBody))
            )
        )
        ...
        nose_tail_displace_phi_clips.append(
            np.concatenate(
                (
                    np.asarray(phi_sin_vector_nose_tail),
...
def compute_allAngle(vectors1, vectors2):
    ...
    for i in range(vectors1.shape[0]):
        ...
        if cos_angle == np.nan or sin_angle == np.nan:
            print(
                "cos_angle==np.nan or sin_angle==np.nan with:",
...
def compute_features_sep_twoMice_Independent(arg, clips_dict):
    ...
    for clip_ind in range(clip_num):
        ...
        body_TO = (
            poseTheOther_clips_alignSelf[clip_ind, :, 0, :]
            - poseTheOther_clips_alignSelf[clip_ind, :, 3, :]
        )
        ...
        head = (
            pose_clips_align[clip_ind, :, 0, :]
            - (
                pose_clips_align[clip_ind, :, 1, :]
                + pose_clips_align[clip_ind, :, 2, :]
            )
            / 2
        )
        head_TO = (
            poseTheOther_clips_alignSelf[clip_ind, :, 0, :]
            - (
                poseTheOther_clips_alignSelf[clip_ind, :, 1, :]
                + poseTheOther_clips_alignSelf[clip_ind, :, 2, :]
            )
            / 2
        )
        if np.any(np.linalg.norm(head) == 0) or np.any(np.linalg.norm(head_TO) == 0):
        ...
        cos_angles_hb_TO, sin_angles_hb_TO, angles_hb_TO = compute_allAngle(
            body_TO, head_TO
        )

        ### FFT of nose
        amplitudes, angles = compute_fft_Nx2(
            pose_clips_align[clip_ind, :, 0, :], first_k=arg.fft_firstK
        )
        amplitudes_TO, angles_TO = compute_fft_Nx2(
            poseTheOther_clips_alignSelf[clip_ind, :, 0, :], first_k=arg.fft_firstK
        )

        ### Displacement
        displace_x, displace_y = (
            pose_clips_align[clip_ind, :, 0, 0],
            pose_clips_align[clip_ind, :, 0, 1],
        )
        displace_x_TO, displace_y_TO = (
            poseTheOther_clips_alignSelf[clip_ind, :, 0, 0],
            poseTheOther_clips_alignSelf[clip_ind, :, 0, 1],
        )

        ### Displacement on polar coordinates
        rho_vector, phi_sin_vector, phi_cos_vector = toPolarCoordinates(
            displace_x, displace_y
        )
        rho_vector_TO, phi_sin_vector_TO, phi_cos_vector_TO = toPolarCoordinates(
            displace_x_TO, displace_y_TO
        )

        ### Contour PCA fft
        amplitudes_contour, angles_contour = compute_fft_NxM(
            cont_clips[clip_ind, :, :], first_k=arg.fft_firstK
        )
        amplitudes_contour_TO, angles_contour_TO = compute_fft_NxM(
            contTheOther_clips[clip_ind, :, :], first_k=arg.fft_firstK
        )

        ### Angle of two mouse body
        body_noAlign = pose_clips[clip_ind, :, 0, :] - pose_clips[clip_ind, :, 3, :]
        body_noAlign_TO = (
            poseTheOther_clips[clip_ind, :, 0, :]
            - poseTheOther_clips[clip_ind, :, 3, :]
        )
        cos_angles_twoBody, sin_angles_twoBody, angles_twoBody = compute_allAngle(
            body_noAlign, body_noAlign_TO
        )

        ### Angle of two mouse head
        head_noAlign = (
            pose_clips[clip_ind, :, 0, :]
            - (pose_clips[clip_ind, :, 1, :] + pose_clips[clip_ind, :, 2, :]) / 2
        )
        head_noAlign_TO = (
            poseTheOther_clips[clip_ind, :, 0, :]
            - (
                poseTheOther_clips[clip_ind, :, 1, :]
                + poseTheOther_clips[clip_ind, :, 2, :]
            )
            / 2
        )
        cos_angles_twoHead, sin_angles_twoHead, angles_twoHead = compute_allAngle(
            head_noAlign, head_noAlign_TO
        )
        ...
        if "body_change_ang" in feature_clips_dict:
            feature_clips_dict["body_change_ang"].append(
                np.concatenate(
                    (
                        np.asarray(sin_angles_bodyChange),
                        np.asarray(cos_angles_bodyChange),
                    )
                )
            )
        else:
            feature_clips_dict["body_change_ang"] = [
                np.concatenate(
                    (
                        np.asarray(sin_angles_bodyChange),
                        np.asarray(cos_angles_bodyChange),
        ...
        displace_Tmouse_nose_to_Rmouse_tail_x = poseTheOther_clips_alignToOther[
            clip_ind, :, 3, 0
        ]
        displace_Tmouse_nose_to_Rmouse_tail_y = poseTheOther_clips_alignToOther[
            clip_ind, :, 3, 1
        ]
        (
            rho_vector_nose_tail_0,
            phi_sin_vector_nose_tail_0,
            phi_cos_vector_nose_tail_0,
        ) = toPolarCoordinates(
            displace_Tmouse_nose_to_Rmouse_tail_x, displace_Tmouse_nose_to_Rmouse_tail_y
        )

        ### Displacement from  refer mouse’s nose to target mouse’s tail .
        displace_Rmouse_nose_to_Tmouse_tail_x = (
            poseTheOther_clips_alignToOther[clip_ind, :, 0, 0]
            + pose_clips_align[clip_ind, :, 3, 0]
        )
        displace_Rmouse_nose_to_Tmouse_tail_y = (
            poseTheOther_clips_alignToOther[clip_ind, :, 0, 1]
            + pose_clips_align[clip_ind, :, 3, 1]
        )
        (
            rho_vector_nose_tail_1,
            phi_sin_vector_nose_tail_1,
            phi_cos_vector_nose_tail_1,
        ) = toPolarCoordinates(
            displace_Rmouse_nose_to_Tmouse_tail_x, displace_Rmouse_nose_to_Tmouse_tail_y
        )

        ### Displacement between one mouse’s nose and the other mouse’s nose.
        displace_Tmouse_nose_to_Rmouse_nose_x = poseTheOther_clips_alignToOther[
            clip_ind, :, 0, 0
        ]
        displace_Tmouse_nose_to_Rmouse_nose_y = poseTheOther_clips_alignToOther[
            clip_ind, :, 0, 1
        ]
        (
            rho_vector_nose_nose,
            phi_sin_vector_nose_nose,
            phi_cos_vector_nose_nose,
        ) = toPolarCoordinates(
            displace_Tmouse_nose_to_Rmouse_nose_x, displace_Tmouse_nose_to_Rmouse_nose_y
        )
        ...
        newFeature = np.ones(
            pose_clips.shape[1]
        )  # the feature of one clip should be a numpy array whose shape is the frame number of the clip
        ...
        contourPCA_fft_ang_clips.append(
            np.concatenate((np.sin(angles_contour), np.cos(angles_contour)))
        )
        ...
        nose_fft_ang_clips_TO.append(
            np.concatenate((np.sin(angles_TO), np.cos(angles_TO)))
        )
        contourPCA_fft_amp_clips_TO.append(np.asarray(amplitudes_contour_TO))
        contourPCA_fft_ang_clips_TO.append(
            np.concatenate((np.sin(angles_contour_TO), np.cos(angles_contour_TO)))
        )
        ...
        two_head_ang_clips.append(
            np.concatenate(
                (np.asarray(sin_angles_twoHead), np.asarray(cos_angles_twoHead))
            )
        )
        ...
        TM_nose_RM_tail_displace_phi_clips.append(
            np.concatenate(
                (
                    np.asarray(phi_sin_vector_nose_tail_0),
        ...
        RM_nose_TM_tail_displace_phi_clips.append(
            np.concatenate(
                (
                    np.asarray(phi_sin_vector_nose_tail_1),
        ...
        nose_nose_displace_phi_clips.append(
            np.concatenate(
                (
                    np.asarray(phi_sin_vector_nose_nose),
        ...
        TM_nose_RM_tail_distance_clips.append(
            compute_allDistance(
                displace_Tmouse_nose_to_Rmouse_tail_x,
                displace_Tmouse_nose_to_Rmouse_tail_y,
            )
        )
        ...
        nose_nose_distance_clips.append(
            compute_allDistance(
                displace_Tmouse_nose_to_Rmouse_nose_x,
                displace_Tmouse_nose_to_Rmouse_nose_y,
            )
        )
        ...
        if number_of_clip_used > arg.max_clips_num:
            print(
                "max_clips_num is set to {} will stop processing more clips".format(
                    arg.max_clips_num
                )
            )
            break
...
def compute_features(arg, norm_pose_clips, info_clips, cont_clips):
    ...
    for clip_ind in range(clip_num):
        ...
        feature_clip = np.concatenate((amplitudes[0:8], np.cos(angles)[0:8]))
        feature_clip = np.concatenate((feature_clip, np.sin(angles)[0:8]))
        raw_feature_clip = np.concatenate(
            (
                norm_pose_clips[clip_ind, :, point, 0],
                norm_pose_clips[clip_ind, :, point, 1],
            )
        )
        ...
        raw_feature_clip = np.concatenate(
            (raw_feature_clip, np.linalg.norm(body, axis=1))
        )
        ...
        raw_feature_clip = np.concatenate(
            (raw_feature_clip, np.linalg.norm(head, axis=1))
        )
...
def visualize_one_cluster(
    ...
    for clip_ind in tqdm(cluster):
        ...
        for frame_ind in range(arg.clip_length):
            ...
            cv2.putText(
                crop,
                "cluster_{}_clip_{}".format(cluster_ind, clip_ind),
                arg.bottomLeftCornerOfText,
                arg.font,
                arg.fontScale,
                arg.fontColor,
...
def cluster(arg, feature_clips_dict, all_info_selected, raw_feature_clips_dict):
    print(
        "number of clips will used after preprocess: {}".format(
            len(feature_clips_dict["displace_x"])
        )
    )
    all_result = []
    all_result_dict = {}
    ...
    for c_arg_dict_idx in range(len(arg.cluster_arg)):
        ...
        print(
            "clustering with {}, with thred:{}".format(
                c_arg_dict["name"], c_arg_dict["thred"]
            )
        )
        ### Normalize and concatenate feature
        feat_norm = c_arg_dict["features_arg"][0]["weight"] * get_normFeature_byArg(
            c_arg_dict["features_arg"][0], feature_clips_dict
        )
        for f_i in range(1, len(c_arg_dict["features_arg"])):
            feat_norm_tmp = c_arg_dict["features_arg"][f_i][
                "weight"
            ] * get_normFeature_byArg(
                c_arg_dict["features_arg"][f_i], feature_clips_dict
            )
        ...
        if isinstance(c_arg_dict["thred"], list):
            print(
                "selecting fcluster threshold with %s metric:"
                % (c_arg_dict["evaluation_metric"])
            )
            score_select = -100000
            thred_select = 0
            for threshold in c_arg_dict["thred"]:
                ...
                if c_arg_dict["evaluation_metric"] == "Davies-Bouldin index":
                    ...
                    all_result_dict[c_arg_dict["name"]] = {
                        "clusters": clusters,
                        "Z": Z,
                        "cluster_result_fcluster": cluster_result_tmp,
                        "leaves": leaves,
                    ...
                    raise
                print(
                    "threshold %d: %s=%f, (score_select=%f,thred_select=%f)"
                    % (
                        threshold,
                        c_arg_dict["evaluation_metric"],
                        score,
        ...
        if arg.DR_method == "tsne":
            ...
            um = umap.UMAP(n_neighbors=5, min_dist=0.3, metric="correlation")
    ...
    if isinstance(c_arg_dict["thred"], list):
        ...
        for threshold in c_arg_dict["thred"]:
            ...
            if c_arg_dict["evaluation_metric"] == "Davies-Bouldin index":
                ...
                score = metrics.calinski_harabasz_score(
                    feature_clips, cluster_result_tmp
                )
                if score > score_select:
                    score_select = score
                    cluster_result = cluster_result_tmp
                ...
                print(
                    "only Calinski-Harabasz Index and Davies-Bouldin index is supported"
                )
                raise
            print(
                "threshold %d: %s=%f, score_select=%f"
                % (threshold, c_arg_dict["evaluation_metric"], score, score_select)
            )
    else:
        threshold = c_arg_dict["thred"]
        cluster_result = hierarchy.fcluster(Z, threshold, "distance")
...
def cluster_bak(arg, feature_clips):
    ...
    g0 = plt.gca()
    dn = hierarchy.dendrogram(Z, above_threshold_color="y", orientation="top", ax=g0)
    # plt.tight_layout()
    leaves = np.asarray(dn["leaves"])
    # # print(leaves)
    ...
    for i in range(last_len):
        ...
        for j in range(1, np.max(cluster_result_fcluster) + 1):
            tmp_dict[last_cluster_name + "_" + sep_name + str(j)] = {
                ...
                "ori_id": [],
                "cluster_name": last_cluster_name + "_" + sep_name + str(j),
            }
        ...
        for k in range(len(cluster_result_fcluster)):
            ...
            tmp_dict[dict_key]["raw_feature_clips"].append(
                cluster_dict["raw_feature_clips"][k]
            )
            tmp_dict[dict_key]["ori_id"].append(cluster_dict["ori_id"][k])
            for ii in range(len(cluster_dict["feature_clips_sep"])):
                tmp_dict[dict_key]["feature_clips_sep"][ii].append(
                    cluster_dict["feature_clips_sep"][ii][k]
                )
...
def visualize_dict(
    ...
    for k in clusters_dict:
        ...
        all_clipID = sorted(
            all_clipID,
            key=lambda clip_id: np.nonzero(
                all_result[cluster_ids[0]]["leaves"] == clip_id
            )[0],
        )
        ...
        for clip_ind in tqdm(all_clipID):
            ...
            for cluster_num_id in cluster_ids:
                ...
                dn = hierarchy.dendrogram(
                    Z, above_threshold_color="y", orientation="top", ax=g0
                )
                plt.tight_layout()
            ...
            for frame_ind in range(arg.clip_length):
                ...
                cv2.putText(
                    crop,
                    "cluster_{}_clip_{}".format(k, clip_ind),
                    arg.bottomLeftCornerOfText,
                    arg.font,
                    arg.fontScale,
                    arg.fontColor,
...
def visualize_allInOneVideo(
    ...
    for feature_key in feature_clips_dict:
        feature_clips_dict_norm[feature_key] = zscoreAll_norm(
            feature_clips_dict[feature_key]
        )
    ...
    for clip_ind_idx in tqdm(range(len(all_result[cluster_ids[0]]["leaves"]))):
        ...
        for cluster_num_id in cluster_ids:
            ...
            ll = g0.plot([xxx, xxx], [0, arg.threshold], linewidth=4, color="red")
            f.savefig(
                "./tmp_image/dendrogram_%s_%d.png" % (video_name_suffix, cluster_num_id)
            )
            ...
            cluster_result_fcluster = all_result[cluster_num_id][
                "cluster_result_fcluster"
            ]
            color = [
                RGB_to_Hex([(50 * c_id) % 255, (80 * c_id) % 255, (110 * c_id) % 255])
                for c_id in cluster_result_fcluster
            ]
            f = plt.figure(figsize=(15, 5))
            plt.scatter(Y[:, 0], Y[:, 1], c=color, cmap=plt.cm.Spectral)
            plt.scatter(
                [Y[clip_ind, 0]], [Y[clip_ind, 1]], marker="*", color="red", s=600
            )
            # plt.title("t-SNE (%.2g sec)" % (t1 - t0))
            # plt.show()
            f.savefig(
                "./tmp_image/tsne_%s_%d.png" % (video_name_suffix, cluster_num_id)
            )
            plt.close()
        ...
        f = plt.figure(figsize=(15, 5))
        plt.barh(
            range(len(feature_distance1)),
            feature_distance1,
            color="rgb",
            tick_label=feature_name,
        )
        f.savefig(
            "./tmp_image/feature_distance1_%s_%d.png"
            % (video_name_suffix, cluster_num_id)
        )
        plt.close()
        f = plt.figure(figsize=(15, 5))
        plt.barh(
            range(len(feature_distance2)),
            feature_distance2,
            color="rgb",
            tick_label=feature_name,
        )
        f.savefig(
            "./tmp_image/feature_distance2_%s_%d.png"
            % (video_name_suffix, cluster_num_id)
        )
        plt.close()
        ...
        for frame_ind in range(arg.clip_length):
            ...
            dend = cv2.imread(
                "./tmp_image/dendrogram_%s_%d.png" % (video_name_suffix, cluster_ids[0])
            )
            ...
            tsne = cv2.imread(
                "./tmp_image/tsne_%s_%d.png" % (video_name_suffix, cluster_ids[0])
            )
            ...
            for cluster_num_id in cluster_ids[1:]:
                dend1 = cv2.imread(
                    "./tmp_image/dendrogram_%s_%d.png"
                    % (video_name_suffix, cluster_num_id)
                )
...
def sort_by_firstTwoFeature(
    cluster_merged_dict, cluster_ids, all_result, max_clips_num=60
):
    cluster_tmp = []
    ...
    cluster_tmp_sorted = sorted(
        cluster_tmp,
        key=lambda clip_cluster: np.nonzero(
            all_result[cluster_ids[0]]["leaves"] == clip_cluster[0]
        )[0],
    )
    ...
    for i in range(len(cluster_tmp_sorted)):
        if len(cluster_ids) > 1:
            list_toAdd = sorted(
                cluster_tmp_sorted[i],
                key=lambda clip_id: np.nonzero(
                    all_result[cluster_ids[1]]["leaves"] == clip_id
                )[0],
            )
        else:
...
def visualize_inOneVideo_sort(
    ...
    all_clipID_sorted, all_clipID_sorted_sep = sort_by_firstTwoFeature(
        cluster_merged_dict, cluster_ids, all_result, max_clips_num=max_clips_num
    )
    ...
    for clip_ind_idx in tqdm(range(len(all_clipID_sorted))):
        ...
        for cluster_num_id in cluster_ids:
            cluster_ids_s += (
                str(all_result[cluster_num_id]["cluster_result_fcluster"][clip_ind])
                + "_"
            )
        ...
        for frame_ind in range(arg.clip_length):
            ...
            cv2.putText(
                crop,
                "cluster_{}_clip_{}".format(cluster_ids_s, clip_ind),
                arg.bottomLeftCornerOfText,
                arg.font,
                arg.fontScale,
                arg.fontColor,
            ...
            cv2.putText(
                dend,
                cluster_ids_name[cluster_ids[0]],
                arg.bottomLeftCornerOfText,
                arg.font,
                arg.fontScale,
                (0, 0, 255),
            ...
            for cluster_num_id in cluster_ids[1:]:
                ...
                cv2.putText(
                    dend1,
                    cluster_ids_name[cluster_num_id],
                    arg.bottomLeftCornerOfText,
                    arg.font,
                    arg.fontScale,
                    (0, 0, 255),
...
def visualize_inSepVideo(
    ...
    _, all_clipID_sorted_sep = sort_by_firstTwoFeature(
        cluster_merged_dict, cluster_ids, all_result, max_clips_num=max_clips_num
    )
    ...
    for iii in range(len(all_clipID_sorted_sep)):
        ...
        for cluster_num_id in cluster_ids:
            cluster_ids_s += (
                str(
                    all_result[cluster_num_id]["cluster_result_fcluster"][
                        all_clipID_sorted[0]
                    ]
                )
                + "_"
        ...
        out_video_path = "{}/cluster_{}_{}.mp4".format(
            gen_video_folder, video_name_suffix, cluster_ids_s
        )
        print(out_video_path)
        fourcc = cv2.VideoWriter_fourcc(*"mp4v")
        out = cv2.VideoWriter(out_video_path, fourcc, 9.0, (1400, 800))
        ...
        for clip_ind_idx in tqdm(range(len(all_clipID_sorted))):
            ...
            for cluster_num_id in cluster_ids:
                ...
                color = [
                    RGB_to_Hex(
                        [(50 * c_id) % 255, (80 * c_id) % 255, (110 * c_id) % 255]
                    )
                    for c_id in cluster_result_fcluster
                ]
                f = plt.figure(figsize=(15, 5))
            ...
            for frame_ind in range(arg.clip_length):
                ...
                dend = cv2.imread(
                    "./tmp_image/dendrogram_%s_%d.png"
                    % (video_name_suffix, cluster_ids[0])
                )
                dend = cv2.resize(
                    dend, (600, 800 - (num_pic_v - 1) * int(800 / num_pic_v))
                )
                cv2.putText(
                    dend,
                    cluster_ids_name[cluster_ids[0]],
...
def write_cluster_result_to_infoDict(
    all_result, all_info_selected, Z_index_in_result_dict=0
):
    ...
    for i in range(len(all_info_selected["info_clips"])):
        ...
        all_info_selected["info_clips"][i]["cluster_id"] = float(
            all_result[0]["cluster_result_fcluster"][i]
        )
...
def process_Z(Z, all_info_selected):
    Z_return = []
    cluster_names = [
        str(int(all_info_selected["info_clips"][i]["cluster_id"]))
        for i in range(len(all_info_selected["info_clips"]))
    ]
...
def save_Z_and_clips(arg, all_result_dict, all_info_selected):
    ...
    print(
        "writing clips information to %s..."
        % (arg.dataPath_for_UI + "/clips_info.json")
    )
...
def visualize_inSepVideo_twoMice(
    arg,
    cluster_ids,
    cluster_ids_name,
    cluster_merged_dict,
    ...
    for cluster_num_id in cluster_ids:
        ...
        color = [
            RGB_to_Hex([(53 * c_id) % 255, (83 * c_id) % 255, (113 * c_id) % 255])
            for c_id in cluster_result_fcluster
        ]
        for i in range(Z.shape[0]):
        ...
        xlim = g0.get_xlim()
        xlim = xlim[1]
        dendrogram_info[cluster_num_id] = {"f": f, "g0": g0, "xlim": xlim}
    ...
    for iii in range(len(all_clipID_sorted_sep)):
        ...
        for clip_ind_idx in tqdm(range(len(all_clipID_sorted))):
            ...
            for cluster_num_id in cluster_ids:
                ...
                ll = g0.plot(
                    [xxx, xxx],
                    [0, arg.cluster_arg[cluster_num_id]["thred"]],
                    linewidth=4,
                    color="red",
                )
                f.savefig(
            ...
            for frame_ind in range(arg.clip_length):
                ...
                for idx_c in range(pose_toDraw.shape[0]):
                    ...
                    cv2.circle(
                        img,
                        center=(
                            int(np.clip(pose_toDraw[idx_c, 0], 0, width)),
                            int(np.clip(pose_toDraw[idx_c, 1], 0, height)),
                        ),
                        radius=1,
                        color=(255, 0, 0),
                        thickness=-1,
                ...
                for idx in range(len(pairs)):
                    if (
                        pose_toDraw[pairs[idx][0], 0] == 0
                        and pose_toDraw[pairs[idx][0], 1] == 0
                    ) or (
                        pose_toDraw[pairs[idx][1], 0] == 0
                        and pose_toDraw[pairs[idx][1], 1] == 0
                    ):
                        continue
                    cv2.line(
                        img,
                        pt1=(
                            int(np.clip(pose_toDraw[pairs[idx][0], 0], 0, width)),
                            int(np.clip(pose_toDraw[pairs[idx][0], 1], 0, height)),
                        ),
                        pt2=(
                            int(np.clip(pose_toDraw[pairs[idx][1], 0], 0, width)),
                            int(np.clip(pose_toDraw[pairs[idx][1], 1], 0, height)),
                        ),
                        color=(255, 0, 0),
                        thickness=2,
                    )
                ...
                M = np.float32(
                    [
                        [1, 0, int(width / 2 - nose[0])],
                        [0, 1, int(height / 2 - nose[1])],
                    ]
                )
                tra = cv2.warpAffine(img, M, (cols, rows))
                ### rotate
                ...
                rot = cv2.warpAffine(tra, M, (cols, rows))
                crop_target = rot[
                    max(0, int(height / 2 - ori_crop_halfHeight)) : int(
                        height / 2 + ori_crop_halfHeight
                    ),
                    max(0, int(width / 2 - ori_crop_halfWidth)) : int(
                        width / 2 + ori_crop_halfWidth
                    ),
                ].copy()
                ...
                M = np.float32(
                    [
                        [1, 0, int(width / 2 - nose_ref[0])],
                        [0, 1, int(height / 2 - nose_ref[1])],
                    ]
                )
                tra = cv2.warpAffine(img, M, (cols, rows))
                ### rotate
                ...
                dend = cv2.resize(
                    ...
                    (
                        dendrogram_x,
                        dendrogram_y - (num_pic_v - 1) * int(dendrogram_y / num_pic_v),
                    ),
                )
                cv2.putText(
                    dend,
                ...
                for cluster_num_id in cluster_ids[1:]:
                    ...
                    dend1 = cv2.resize(
                        dend1, (dendrogram_x, int(dendrogram_y / num_pic_v))
                    )
                    cv2.putText(
                        dend1,
                        cluster_ids_name[cluster_num_id],
                    ...
                    tsne = cv2.resize(
                        tsne, (dendrogram_x, int(dendrogram_y / num_pic_v))
                    )
                    dend = cv2.vconcat((dend, tsne))
                img = cv2.hconcat((img, dend))
...
def init_Videos_frameLabelDict(arg, gt_dict):
    print(
        "initialiing frame label list dict (%d videos in tatal)..."
        % (len(arg.videodir))
    )
    print(
        "note: the key of the dict is generated with path in arg.videodir. please make sure the path can be correctly re."
    )
...
def label_videos(arg, all_result, cluster_ids, info_clips, gt_dict):
    ...
    for clip_idx in range(len(info_clips)):
        ...
        for c_id in cluster_ids:
            clip_label = (
                clip_label
                + str(all_result[c_id]["cluster_result_fcluster"][clip_idx])
                + "_"
            )
    ...
    for video_key in gt_dict:
        ...
        for second_idx in range(len(gt_dict[video_key])):
            ...
            if second_label in all_cluster_frame_dict:
                all_cluster_frame_dict[second_label] += [
                    i for i in range(second_idx * 30, (second_idx + 1) * 30)
                ]
            else:
                all_cluster_frame_dict[second_label] = [
                    i for i in range(second_idx * 30, (second_idx + 1) * 30)
        ...
        print(
            total_predict,
            video_key,
            [[k, pred_cluster_dict[k]] for k in pred_cluster_dict] + [0],
        )
        same_as_gt_count += max([pred_cluster_dict[k] for k in pred_cluster_dict] + [0])

    print(
        "total prediction: %d \nprediction same as gt:%d"
        % (total_predict, same_as_gt_count)
    )
...
def get_result_for_evaluation(
    arg,
    all_result_dict,
    all_info_selected,
    feature_clips_dict,
    video_id_forEvaluation,
    c_arg_dict_idx_forFeature,
):
    if isinstance(video_id_forEvaluation, int):
        video_id_forEvaluation = str(video_id_forEvaluation)
    ...
    for f_i in range(1, len(c_arg_dict["features_arg"])):
        feat_norm_tmp = c_arg_dict["features_arg"][f_i][
            "weight"
        ] * get_normFeature_byArg(c_arg_dict["features_arg"][f_i], feature_clips_dict)
        feat_norm = np.concatenate([feat_norm, feat_norm_tmp], axis=1)
    ...
    for i in range(len(all_info_selected["info_clips"])):
        if (
            all_info_selected["info_clips"][i]["clip_id"].split("_")[0]
            == video_id_forEvaluation
        ):
...
def evaluate(
    ...
    for e_i in range(len(gt_key)):
        ...
        evaluate_dict = get_result_for_evaluation(
            arg,
            all_result_dict,
            all_info_selected,
            feature_clips_dict,
    ...
    print(
        "Adjusted Rand index of random assignments:%f"
        % metrics.adjusted_rand_score(gt, pred_random)
    )
    ...
    print(
        "Mutual Information based scores:%f"
        % metrics.adjusted_mutual_info_score(gt, pred)
    )

    ## 0.0 is as bad as it can be, 1.0 is a perfect score.
    print(
        "Homogeneity, completeness and V-measure:{}".format(
            metrics.homogeneity_completeness_v_measure(gt, pred)
        )
    )
    ...
    print("#######################################################")
    return {
        "Adjusted Rand index": metrics.adjusted_rand_score(gt, pred),
