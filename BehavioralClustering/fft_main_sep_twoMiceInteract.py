...
clips_dict["poseTheOther_clips"] = fft_utils.left_right(
    clips_dict["poseTheOther_clips"]
)

### remove limbs that is not correct
clips_dict["pose_clips"] = fft_utils.correctLimbs(clips_dict["pose_clips"])
clips_dict["poseTheOther_clips"] = fft_utils.correctLimbs(
    clips_dict["poseTheOther_clips"]
)

### align Pose
clips_dict["pose_clips_align"] = fft_utils.align_poses_self(clips_dict["pose_clips"])
clips_dict["poseTheOther_clips_alignSelf"] = fft_utils.align_poses_self(
    clips_dict["poseTheOther_clips"]
)
clips_dict["poseTheOther_clips_alignToOther"] = fft_utils.align_poses_toTheFirst(
    clips_dict["pose_clips"], clips_dict["poseTheOther_clips"]
)
...
for c_arg_dict_idx in range(len(arg.cluster_arg)):
    ...
    print(
        "clustering with %s, with thred:%f" % (c_arg_dict["name"], c_arg_dict["thred"])
    )
    ### Normalize and concatenate feature
    feat_norm = c_arg_dict["features_arg"][0][
        "weight"
    ] * fft_utils.get_normFeature_byArg(
    ...
    clusters, Z, cluster_result_fcluster, leaves = fft_utils.cluster_bak1(
        c_arg_dict, feature_clips_forCluster
    )
...
all_result, all_result_dict = fft_utils.cluster(
    arg, feature_clips_dict, all_info_selected, raw_feature_clips_dict
)
...
name = [
    arg.cluster_arg[c_arg_dict_i]["name"]
    for c_arg_dict_i in range(len(arg.cluster_arg))
]

cluster_merged_dict = {}
print(
    "clustering using:",
    [arg.cluster_arg[c_arg_dict_i]["name"] for c_arg_dict_i in arg.cluster_ids],
)
for data_i in range(len(all_result[0]["cluster_result_fcluster"])):
    cluster_key = ""
    for c_i in arg.cluster_ids:
        cluster_key = (
            cluster_key + str(all_result[c_i]["cluster_result_fcluster"][data_i]) + "_"
        )
...
################ visualize  ##################
fft_utils.visualize_inSepVideo_twoMice(
    arg,
    arg.cluster_ids,
    name,
    cluster_merged_dict,
    ...
    all_info_selected["cont_clips"],
    raw_feature_clips_dict,
    feature_clips_dict,
    video_name_suffix=arg.video_name_suffix,
    gen_video_folder=gen_video_folder,
    max_clips_num=arg.max_clips_num,
)
